module.exports = {
    'en': {
        preferences: 'Preferences',
        about: 'About Mozu Studio',
        hide: 'Hide Mozu Studio',
        hideothers: 'Hide Others',
        service: 'Services',
        reload_app: 'Reload App',
        quit: 'Quit',
        window: 'Window',
        minimize: 'Minimize',
        close: 'Close Window',
        file: 'File',
        edit: 'Edit',
        help: 'Help',
        import: 'Import',
        export_flux_task: 'Export FLUX task',
        save_scene: 'Save Scene',
        undo: 'Undo',
        duplicate: 'Duplicate',
        scale: 'Scale',
        rotate: 'Rotate',
        reset: 'Reset',
        align_center: 'Align Center',
        clear_scene: 'Clear Scene',
        machines: 'Machines',
        add_new_machine: 'Machine Setup',
        help_center: 'Help Center',
        contact: 'Contact Us',
        tutorial: 'Start Delta Family Printing Tutorial',
        forum: 'Community Forum',
        software_update: 'Software Update',
        bug_report: 'Bug Report',
        dashboard: 'Dashboard',
        machine_info: 'Machine Info',
        toolhead_info: 'Toolhead Info',
        change_material: 'Change Printing Material',
        run_leveling: 'Run Auto Leveling',
        commands: 'Commands',
        update_firmware: 'Update Firmware',
        update_delta: 'Machine Firmware',
        update_toolhead: 'Toolhead Firmware',
        set_as_default: 'Set as Default',
        calibrate_origin: 'Calibrate Origin ( Home )',
        calibrate_beambox_camera: 'Calibrate Camera',
        movement_test: 'Run Movement Test',
        turn_on_laser: 'Turn On Scanning Laser',
        auto_leveling_clean: 'Run Auto Leveling with Clean Data',
        set_toolhead_temperature: 'Set Toolhead Temperature',
        sign_in: 'Sign In',
        sign_out: 'Sign Out',
        account: 'Account',
        my_account: 'My Account',
        download_log: 'Download Logs',
        download_log_canceled: 'Log download canceled',
        download_log_error: 'Unknown error occurred, please try it again later',
        log: {
            network: 'Network',
            hardware: 'Hardware',
            discover: 'Discover',
            usb: 'USB',
            camera: 'Camera',
            cloud: 'Cloud',
            player: 'Player',
            robot: 'robot'
        },
        link: {
            help_center: 'https://helpcenter.flux3dp.com/',
            contact_us: 'https://flux3dp.zendesk.com/hc/en-us/requests/new',
            forum: 'https://forum.flux3dp.com/',
            downloads: 'https://flux3dp.com/downloads/'
        }

    },
    'zh-tw': {
        preferences: '偏好設定',
        about: '關於 Mozu Studio',
        hide: '隱藏 Mozu Studio',
        hideothers: '隱藏其他',
        service: '服務',
        reload_app: '重新整理',
        quit: '離開',
        window: '視窗',
        minimize: '最小化',
        close: '關閉視窗',
        file: '檔案',
        edit: '編輯',
        help: '說明',
        import: '匯入',
        export_flux_task: '匯出工作',
        save_scene: '匯出場景',
        undo: '復原',
        duplicate: '重製',
        scale: '縮放',
        rotate: '旋轉',
        reset: '重設',
        align_center: '置中',
        clear_scene: '清除場景',
        machines: '機器',
        add_new_machine: '新增或設定機器',
        help_center: '說明中心',
        contact: '聯絡我們',
        tutorial: 'Delta Family 列印教學',
        forum: '社群論壇',
        software_update: '軟體更新',
        bug_report: '錯誤回報',
        dashboard: '儀表版',
        machine_info: '機器資訊',
        toolhead_info: '工具頭資訊',
        change_material: '更換線料',
        run_leveling: '校正平台',
        commands: '指令',
        update_firmware: '韌體更新',
        update_delta: '機器韌體',
        update_toolhead: '工具頭韌體',
        set_as_default: '設為預設',
        calibrate_origin: '回歸原點',
        calibrate_beambox_camera: '校正相機',
        movement_test: '執行運動測試',
        turn_on_laser: '打開掃描雷射',
        auto_leveling_clean: '校正平台（清除原始資料）',
        set_toolhead_temperature: '設定列印工具頭溫度',
        sign_in: '登入',
        sign_out: '登出',
        account: '帳號',
        my_account: '我的帳號',
        download_log: '匯出機器日誌',
        download_log_canceled: '取消日誌下載',
        download_log_error: '不明錯誤發生，請稍候再試一次',
        log: {
            network: 'Network',
            hardware: 'Hardware',
            discover: 'Discover',
            usb: 'USB',
            camera: 'Camera',
            cloud: 'Cloud',
            player: 'Player',
            robot: 'robot'
        },
        link: {
            help_center: 'https://helpcenter.flux3dp.com/',
            contact_us: 'https://flux3dp.zendesk.com/hc/zh-tw/requests/new',
            forum: 'https://forum.flux3dp.com/',
            downloads: 'https://tw.flux3dp.com/downloads/'
        }
    },
    'zh-cn': {
        preferences: '偏好设置',
        about: '关于',
        hide: '隐藏',
        hideothers: '隐藏其他',
        service: '服务',
        reload_app: '刷新',
        quit: '离开',
        window: '窗口',
        minimize: '最小化',
        close: '关闭窗口',
        file: '文件',
        edit: '编辑',
        help: '说明',
        import: '导入',
        export_flux_task: '导出工作',
        save_scene: '导出场景',
        undo: '复原',
        duplicate: '重制',
        scale: '缩放',
        rotate: '旋转',
        reset: '重设',
        align_center: '置中',
        clear_scene: '清除场景',
        machines: '机器',
        add_new_machine: '添加或设置机器',
        help_center: '说明中心',
        contact: '联系我们',
        tutorial: 'Delta Family 打印教学',
        forum: '社区论坛',
        software_update: '软件更新',
        bug_report: '下载错误报告',
        dashboard: '仪表版',
        machine_info: '机器信息',
        toolhead_info: '工具头信息',
        change_material: '更换线料',
        run_leveling: '校正平台',
        commands: '指令',
        update_firmware: '固件更新',
        update_delta: '机器固件',
        update_toolhead: '工具头固件',
        set_as_default: '设为默认',
        calibrate_origin: '回归原点',
        calibrate_beambox_camera: '校正相机',
        movement_test: '运行运动测试',
        turn_on_laser: '打开扫描雷射',
        auto_leveling_clean: '校正平台（清除原始数据）',
        set_toolhead_temperature: '设置打印工具头温度',
        sign_in: '登录',
        sign_out: '注销',
        account: '帐号',
        my_account: '我的帐号',
        download_log: '导出机器日志',
        download_log_canceled: '取消日志下载',
        download_log_error: '不明错误发生，请稍候再试一次',
        log: {
            network: 'Network',
            hardware: 'Hardware',
            discover: 'Discover',
            usb: 'USB',
            camera: 'Camera',
            cloud: 'Cloud',
            player: 'Player',
            robot: 'robot'
        },
        link: {
            help_center: 'https://helpcenter.flux3dp.com/',
            contact_us: 'https://flux3dp.zendesk.com/hc/zh-tw/requests/new',
            forum: 'https://forum.flux3dp.com/',
            downloads: 'http://52mozu.com'
        }
    },
};
