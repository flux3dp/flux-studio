  ...
  platform = platform + (process.arch == 'ia32' ? '32' : '64');
   ...
   * @param  {Function} onProgress - called when download in progress. Callback arguments: data, totalReceivingData, totalSize
   ...
   */
  updater.prototype.download = function(cb, newManifest, onProgress){
    ...
    pkg.on('response', function(response){
      ...
      if(response && response.headers && response.headers['content-length']){
        pkg['content-length'] = parseInt(response.headers['content-length'], 10);

        // on receving data
        pkg.on('data', function(contentLength, data){
          downloadSize = downloadSize + data.length;
          onProgress(data, downloadSize, contentLength);
        }.bind(null, pkg['content-length']));
      }
    });
  ...
  updater.prototype.getAppExec = function(){
    ...
    exec.win64 = exec.win32;
    exec.linux32 = exec.win32;
    exec.linux64 = exec.win32;
  ...
  var pUnpack = {
     ...
     * @private
     */
    osx64: function(filename, cb, manifest, temporaryDirectory){
      var args = arguments,
  ...
  var pRun = {
     ...
     * @private
     */
    osx64: function(appPath, args, options){
      //spawn
      args = [appPath];
    ...
    win32: function(appPath, args, options, cb){
      var paths = path.parse(appPath);
      fs.chmodSync(appPath, 0755);
      options.cwd = paths.dir;
      appPath = paths.base;
