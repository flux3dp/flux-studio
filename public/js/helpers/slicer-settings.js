define([
    ...
    let cura2mapping = {
        ...
        fill_density                        : { key: 'infill_line_distance', fn: (v) => { v = v || '20%'; return 0.4 * 100 * 2 / parseFloat(v.toString().replace('%', '')) } },
        ...
        support_material_speed              : { key: ['speed_support_infill', 'speed_support_interface'], fn: (v) => {
                                                                                                             return [parseFloat(v), parseFloat(v) / 1.5];
                                                                                                          } },
    ...
    SlicerSettings.prototype.toExpert = function(customString = '', slicer = 'slic3r') {
        ...
        function slic3r() {
            var keys = Object.keys(self),
                custom = ( customString || self.custom ).split('\n'),
                _entry, lineNumber;
            ...
            for(var i = 0; i < keys.length; i++) {
                ...
                lineNumber = -1;
                for(var j = 0; j < custom.length && lineNumber < 0; j++) {
                    if (custom[j].indexOf(key + ' ') == 0) lineNumber = j;
                    if (custom[j].indexOf(key + '=') == 0) lineNumber = j;
                }
                ...
                if(lineNumber >= 0) {
                    ...
                    if(hiddenPresets.indexOf(key) === -1) {
                        custom.push(_entry);
                    }
                }
            }
        ...
        function cura2() {
            var keys = Object.keys(self),
                customCura2 = ( customString || self.customCura2 ).split('\n'),
                _entry, lineNumber;

            keys = keys.filter((key) => hiddenPresets.indexOf(key) === -1);

            for(var i = 0; i < keys.length; i++) {
                let key = keys[i], value = self[key];
                ...
                if (allCura2Keys.indexOf(key) < 0) continue;

                _entry = key + ' = ' + value;

                lineNumber = -1;
                for(var j = 0; j < customCura2.length && lineNumber < 0; j++) {
                    if (customCura2[j].indexOf(key + ' ') == 0) lineNumber = j;
                    if (customCura2[j].indexOf(key + '=') == 0) lineNumber = j;
                }

                if(lineNumber >= 0) {
                    customCura2[lineNumber] = _entry;
                }
                else {
                    if(hiddenPresets.indexOf(key) === -1) {
                        customCura2.push(_entry);
                    }
                }
            }
        ...
        if(result.indexOf("perimeter") == 0) throw new Error();
        return result;
    }
